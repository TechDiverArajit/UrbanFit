        ...
        <>
            <div className="flex my-10 mx-3 w-[316px] h-[165px] bg-white">
                ...
                <h1 className="select-none flex "><span className="font-semibold mr-2" >Quantity: </span>
                    <div onClick={()=>dispatch({type:"MINUS",payload:obj})} className="w-[30px] bg-amber-100 text-center h-6 border-1 select-none">-</div> <div className=" text-center select-none w-9 h-6 border-1">{obj.quantity} </div> <div onClick={()=>dispatch({type:"PLUS",payload:obj})} className="text-center w-[30px] bg-amber-100 select-none h-6 border-1">+</div>
                    </h1>
                <h1 cla >Size:{size}</h1>
